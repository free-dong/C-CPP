...
// *********************************************
int detectHuman(arguments args)
{
	Redis* r = new Redis();
	if (args.mode == 3)
	{
		if (!r->connect(args.redis_host, args.redis_port))
		{
	...
	//检测目标标签
	int flag_detect = 0; //是否检测到目标:0,1,2:没检测到，检测到入侵外部区域，检测内部
	string savePath = "save_images";
	makedir(savePath);
	//定义集合：用来监测是否有新的目标
	set<int> set_target;
	//定义内部检测集合：用来监测是否有新的目标
	set<int> set_targetIn;
	...
	vector<Center>centers; //目标中心
	vector<Center>old_centers;
	//end**********************************************
	
	//开始处理每一帧
    bool stop=false;
    while(capture.read(frame)){
		for (int i = 0; i < args.frame_skip; i++)//跳帧
		...
		//2.添加四边形（区域入侵检测）
		Points quad_gt;
		quad_gt.A = getPoint(args.point_A, 0, 0);//左上
		quad_gt.B = getPoint(args.point_B, 0, 0);//右上
		quad_gt.C = getPoint(args.point_C, 0, 0);//左下
		quad_gt.D = getPoint(args.point_D, 0, 0);//右下				
		Points quad_in;//内部
		quad_in.A = getPoint(args.point_A, 10, 10);//左上
		quad_in.B = getPoint(args.point_B, -10, 10);//右上
		quad_in.C = getPoint(args.point_C, 50, -10);//左下
		quad_in.D = getPoint(args.point_D, -10, -10);//右下			
		// 绘制四条边
		pltRect(quad_gt, frame, 1, 100);
		// 绘制内部四条边
		pltRect(quad_in, frame, 0, 255);
		
		// 绘制四边形,填充
		// Point PointArray[1][4];
		// PointArray[0][0] = quad_gt.A; PointArray[0][1] = quad_gt.B;
		// PointArray[0][2] = quad_gt.D; PointArray[0][3] = quad_gt.C;
		//cv::fillConvexPoly(frame, PointArray[0], 4, Scalar(255)); //填充多边形

		//ppt[]要同时添加两个多边形顶点数组的地址头
		//const Point* pts[] = {PointArray[0]};
		//npts[]要定义每个多边形的定点数
		//int npts[] = {4};
		//cv::polylines(frame,pts,npts,2,true,Scalar(255),2,8,0); //绘制多边形
		...
		for (auto tb : frameTrackingResult)
		{
			...
			//计算目标中心
			// Center center;
			// center.x = tb.box.x + tb.box.width / 2;
			// center.y = tb.box.y + tb.box.height / 2;
			// center.id = tb.id;
			// centers.push_back(center);
			// Point cen(center.x, center.y);

			//判断是否有入侵，并在入侵区域 显示 目标检测框，只记录第一次入侵
			...
			//判断是否有入侵内部
			if (dete_intersection(quad_in, tb.box) && (label == detectLabel))
				//若检测到入侵内部 且 目标为“detectLabel”
			{	
				//显示 目标检测框
				cv::rectangle(frame, tb.box, randColor[tb.id % CNUM], 2, 8, 0);
				//显示 监测目标类别文字
				...
				//更新集合：用来监测是否有新的目标
				if (set_targetIn.count(tb.id) == 0)
				{
					flag_detect = 2;
					set_targetIn.insert(tb.id);
				}
			}
		...
		else // read image, draw results and show them
		{
			...
			if (waitKey(10) == 27)//按下ESC(ASCII码为27),则跳出循环,否则,则跳出循环
				break;
		}
		if (flag_detect != 0)
		{	//若检测到入侵目标
			...
			string str_data(data_encode.begin(), data_encode.end());
			str_data = base64_encode((char*)str_data.c_str(), str_data.size());	
			curl_post_data(args.httpd_ip,args.httpd_port,"picname","time","frametime",str_data);
			//post_http(frame,args.httpd_ip,args.httpd_port,"tb.id" + "_3", "tb.id","3");

			//图像解码
			// Mat show=imdecode(data_encode, CV_LOAD_IMAGE_COLOR);
			// imshow("picture",show);
			// cv::waitKey(0);
